<template>
  <div class="detailedRecord">
    <div class="title-box">
      ...
      <span class="title">维修保养记录</span>
    </div>
    <div class="record-item" v-for="(item, index) in detailList" :key="index">
      <p class="date-box">
        <span class="date">{{ item.repairdate }}</span>
        <span
          class="maintain tag"
          :class="item.type.indexOf('修') != -1 ? 'repair' : ''"
          >{{ item.type }}</span
        >
      </p>
      <div class="desc">
        <div class="desc-title">项目:</div>
        <div class="desc-content">
          <p>{{ item.content }}</p>
        </div>
      </div>
      <div class="desc">
        <div class="desc-title">材料:</div>
        <div class="desc-content">
          <p>{{ item.material }}</p>
        </div>
      </div>
